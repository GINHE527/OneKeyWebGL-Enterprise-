...
//初始构造
void SElImagePicker::Construct(const FArguments& InArgs)
{
		...
		[
			...
			[
				...
				[
					...
					[
						SNew(SBox)
						.WidthOverride(this, &SElImagePicker::GetImageWidth)
						.HeightOverride(this, &SElImagePicker::GetImageHeight)
						[
							SNew(SEnableBox)
							[
								SNew(SImage)
								.Image(this, &SElImagePicker::GetImage)
								.ToolTipText(this, &SElImagePicker::GetImageTooltip)
							]
						]
					]
				]
			...
			[
				...
				.OnClicked( FOnClicked::CreateSP(this, &SElImagePicker::OnPickFile) )
				.ContentPadding( 2.0f )
	...
	if(HorizontalBox.IsValid() && DefaultImagePath.Len() > 0)
	{
		HorizontalBox->AddSlot()
			...
			[
				...
				.OnResetToDefault(FSimpleDelegate::CreateSP(this, &SElImagePicker::HandleResetToDefault))
				.OnGetResetToDefaultText(FOnGetResetToDefaultText::CreateSP(this, &SElImagePicker::GetResetToDefaultText))
				.DiffersFromDefault(this, &SElImagePicker::DiffersFromDefault)
			];
	}
...
FVector2D SElImagePicker::GetConstrainedImageSize() const
{
...
FOptionalSize SElImagePicker::GetImageWidth() const
{
	return GetConstrainedImageSize().X;
}

FOptionalSize SElImagePicker::GetImageHeight() const
{
	return GetConstrainedImageSize().Y;
}

void SElImagePicker::ApplyImageWithExtenstion(const FString& Extension)
{
	...
	UE_LOG(LogTemp,Warning,TEXT("Extend is %s"),*TargetImagePath);
	ApplyImage();
}

//这段话没问题
void SElImagePicker::ApplyFirstValidImage()
{
...
void SElImagePicker::ApplyImage()
{
	//清空当前的错误信息
	...
	{
		//检查是否存在，如果存在
		UE_LOG(LogTemp, Warning, TEXT("Begin to apply image ,Path is :%s"), *TargetImagePath);
...
void SElImagePicker::ApplyImage(const FString& ImagePath)
{
...
void SElImagePicker::ApplyPlaceholderImage()
{
	if (ImageBrush.IsValid())
	{
		FSlateApplication::Get().GetRenderer()->ReleaseDynamicResource(*ImageBrush.Get());
...
TSharedPtr< FSlateDynamicImageBrush > SElImagePicker::LoadImageAsBrush( const FString& ImagePath )
{
...
FReply SElImagePicker::OnPickFile()
{
	...
	if ( DesktopPlatform )
	{
		...
		if (DesktopPlatform->OpenFileDialog(ParentWindowHandle, FText::Format(LOCTEXT("ImagePickerDialogTitle", "Choose a {0} file"), Title).ToString(), DefaultPath, TEXT(""), Filter, EFileDialogFlags::None, OutFiles))
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("PICKED BUTTON TO SELECT A NEW PNG %s"), *SourceImagePath);
			if (SourceImagePath != TargetImagePath)
			{
				UE_LOG(LogTemp, Warning, TEXT("PICKED BUTTON TO SELECT A NEW PNG ,THE PATH IS %s"), *SourceImagePath);
				TargetImagePath = SourceImagePath;
				//Sett->ImagePath = SourceImagePath;
...
void SElImagePicker::HandleResetToDefault()
{
...
FText SElImagePicker::GetResetToDefaultText() const
{
	return FText::FromString(DefaultImagePath);
}

bool SElImagePicker::DiffersFromDefault() const
{
